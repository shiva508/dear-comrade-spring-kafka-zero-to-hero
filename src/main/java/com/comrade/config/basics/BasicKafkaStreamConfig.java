package com.comrade.config.basics;
...
@Configuration
@ConditionalOnExpression("${enable.basic-kafka}")
public class BasicKafkaStreamConfig {
    @Bean
    public Supplier<Message<News>> nameProducer(){
    ...
    @Bean
    public Function<KStream<String,News>,KStream<String,String>> wordCountProcess(){
        return namesStream->namesStream
                .mapValues(news -> news.name().toUpperCase())
                .flatMapValues(s -> Arrays.asList(s.split("\\s")))
                .selectKey((key, news) ->news);
    }
    @Bean
    public Function<KStream<String,String>,KStream<String,String>> wordCountHandler(){
        return namesStream->namesStream
                                        .groupByKey()
